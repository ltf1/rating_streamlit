...
def plot_top_ten_abs_shap_values(program_name, mean_shap_values):
    ...
    plt.ylabel('Feature')
    plt.title(f'Top Ten Values for: {program_name}')
...
def main():
    ...
    df_results = df_features.groupby(['program_name', "match_start_date", "channel_id_str", "group_name",
                                 "match_stage_8", "match_stage_original" ])[["predictions_euro_2020", "predictions_world_cup"]].mean().sort_values(by='match_start_date', ascending=True).reset_index()
    ...
    selected_match_stage = st.sidebar.selectbox("Select Match Stage", df_results.match_stage_original.unique())
    
    if selected_match_stage == "GROUP STAGE":
        selected_group = st.sidebar.selectbox("Select Group", df_results.group_name.unique())
        filtered_match_stage = df_results[(df_results.match_stage_original == selected_match_stage) & (df_results.group_name == selected_group)]
    else:
        filtered_match_stage = df_results[df_results.match_stage_original == selected_match_stage]
